...
def test_get_post_url_env(mocker: MockerFixture):
    mocker.patch.dict(os.environ, {'CIRCLE_PULL_REQUEST': 'https://github.com/khulnasoft-lab/xsoar-docs/pull/9'})
    assert get_post_url() == 'https://api.github.com/repos/khulnasoft-lab/xsoar-docs/issues/9/comments'


def test_get_post_url_comment(mocker: MockerFixture):
    mocker.patch('subprocess.check_output', return_value='Update docs (#1111)')
    assert get_post_url() == 'https://api.github.com/repos/khulnasoft-lab/xsoar-docs/issues/1111/comments'
